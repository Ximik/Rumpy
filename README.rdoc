...
+lang.yml+      ::  Your bot's responces. Append to existing keys more answers and use them like <tt>@lang['someanswer]</tt>. There *MUST* be at least 3 keys:
  * +hello+ is used when somebody adds bot
  * +stranger+ is used when somebody trying to speak with bot without authorization
  * +authorized+ is used when bot gets authorization.
